...
input double hedgeRiskAmountPerTrade = 300; // Hedging risk amount
...
void HedgeBuy(double sellNeckLine) {
    double entryPrice = iClose(_Symbol, PERIOD_CURRENT, 0);
    double tp = sellNeckLine;
    double sl = entryPrice + (entryPrice - tp) * tpMultiplier;

    double volumes[];
    CalculateVolume(hedgeRiskAmountPerTrade, entryPrice, sl, _Symbol, volumes);
...
void HedgeSell(double buyNeckLine) {
    double entryPrice = iClose(_Symbol, PERIOD_CURRENT, 0);
    double tp = buyNeckLine;
    double sl = entryPrice - (tp - entryPrice) * tpMultiplier;

...
int OnInit() {
    ...
    if (atrHandle == INVALID_HANDLE) {
        Print("Error initializing ATR for ", _Symbol);
        return INIT_FAILED;
    }
...
void OnTick() {
    ...
    Comment("hasOpenPositions: " + (string)hasOpenPositions() + "\ncanTrade: " + (string)canTrade() + "\nATR Value: " + DoubleToString(atrValue) + "\nATR x Multiplier: " + DoubleToString(atrValue * atrMultiplier) + "\nDistance From Highest Point: " + DoubleToString(diffBetweenCurrentPriceAndHighestPoint) + "\nDistance From Lowest Point: " + DoubleToString(diffBetweenCurrentPriceAndLowestPoint));
    ...
    // if (diffBetweenCurrentPriceAndHighestPoint >= atrValue * atrMultiplier && bias != 2) {
    //     Buy();
    // } else if (diffBetweenCurrentPriceAndLowestPoint >= atrValue * atrMultiplier && bias != 1) {
    //     Sell();
    // }
...
double GetATRValue()
{
    ...
    CopyBuffer(atrHandle, 0, 0, 1, atr_values);
